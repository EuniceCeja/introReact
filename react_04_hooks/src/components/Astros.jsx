...
const Astros = () => {
    ...
    const fetchAstros = () => {
        fetch(ASTROS_URL)
            ...
            .then(data => setAstros(data.people)) //Para que la información viva dentro del componente actualizado, es necesario traer setAstros y pasar los datos de `people` de la API
            .catch(error => console.error(error))
    }

    console.log(astros); //Se muestra en consola del navegador

    //Enviar la información de la API a las cards utilizando .map sobre el array `astros`
    return (
        <>
            {
                //`astro` funciona como los elementos que se recorren en el array `astros` y .map recibe dichos elementos, más un index y una funcón tipo callback
